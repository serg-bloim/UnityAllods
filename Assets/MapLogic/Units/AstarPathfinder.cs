...
class AstarPathfinder : ISearchAlgorithm<Vector2i>
{
    ...
    private bool isStaticOnly;
    private bool found = false;
    private int limit;
    private SearchContext<Vector2i> ctx;

    public AstarPathfinder(MapUnit mapUnit, float distance, bool isStaticOnly, int limit = -1)
    {
    ...
    public List<Vector2i> FindPath(MapUnit unit, int x, int y, bool staticOnly, int limit = -1)
    {
    ...
    public ISearchResult<Vector2i> search(SearchContext<Vector2i> ctx)
    {
        ...
        limit = 16;
        this.ctx = ctx; 
        var path = FindPath(mapUnit, start.x, start.y, isStaticOnly, limit);
